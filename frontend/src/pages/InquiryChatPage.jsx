...
export default function InquiryChatPage() {
  ...
  useEffect(() => {
    inquiriesAPI.getMessages(id)
      .then(d => {
        setData(d)
        setMessages(d.messages || [])
      })
      .catch(() => navigate('/my-inquiries'))
  }, [id, navigate])
  ...
  const send = async () => {
    ...
    setError('')
    try {
      const msg = await inquiriesAPI.sendMessage(id, input.trim())
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f0f0f', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: '#888' }}>
            {data?.listing_locality} · {data?.status === 'closed' ? 'Closed' : 'Active'}
          </p>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {messages.length === 0 && (
          <p style={{ color: '#666', textAlign: 'center', marginTop: 40 }}>No messages yet. Say hello!</p>
        )}
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
              <div style={{
                ...
                {!isMe && (
                  <p style={{ margin: '0 0 4px', fontSize: 11, color: '#a855f7', fontWeight: 600 }}>Owner</p>
                )}
